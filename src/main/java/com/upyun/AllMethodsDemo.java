...
public class AllMethodsDemo {
    public static void main(String[] args) throws Exception {
        RestManager manager = new RestManager("服务名", "操作员账号", "操作员密码");
        String dirUri = "/"; //目录路径
//        String source = "/"; //源文件  /<source_to_dir>
//        String uri = "/";  //目标路径    /<save_as_dir>

//        toGetFilesList(dirUri,manager); //统计目录下的文件和目录 + 数量
...
//        moveDir(uri,source,manager); //移动文件夹
    ...
    private static void copyDir(String uri, String source, RestManager manager) throws Exception {
        ...
        for (String filePath : fileList) {
            String sourceFileUri = "/" + bucketName + URLDecoder.decode(filePath,"UTF-8");
            String toFileUri = "/" + bucketName + URLDecoder.decode(uri + filePath,"UTF-8");
